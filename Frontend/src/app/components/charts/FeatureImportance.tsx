...
import { FEATURE_IMPORTANCE } from "../../mock/model";

export function FeatureImportance() {
  return (
    <ResponsiveContainer width="100%" height={280}>
      <BarChart data={FEATURE_IMPORTANCE} layout="vertical" margin={{ left: 20, right: 30, top: 5, bottom: 5 }}>
        ...
        <Bar dataKey="importance" radius={[0, 4, 4, 0]} barSize={18}>
          {FEATURE_IMPORTANCE.map((_, i) => <Cell key={`cell-${i}`} fill={i === 0 ? "#E67E22" : "#163A70"} opacity={1 - i * 0.06} />)}
        </Bar>
      </BarChart>
    </ResponsiveContainer>
  );
